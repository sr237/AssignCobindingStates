# assign_cobinding_states.R — fixed to match Python output exactly
#
# FIXES vs previous R version:
#    FIX 1: get_per_orange_for_each_footprint() completely rewritten.
#           Python counts RUNS OF CONSECUTIVE UPPERCASE 'F' only.
#           Denominator = total window length (not run length).
#           Any non-F char resets the F-run counter.
#    FIX 2: get_count_and_percentage_methylation() — 'M' is neither
#           upper nor lower in Python (c==c.lower() is False for 'M'
#           because 'M'.lower()=='m' != 'M'), so M is correctly skipped.
#           R regex [A-LN-Z] already excludes M — preserved.
#    All indexing, 150bp trim, read_id, edge logic unchanged (were correct).


# ── SECTION 1: helper functions ───────────────────────────────────────────────

count_m_at_start_end <- function(fp_str) {
  chars <- strsplit(fp_str, "")[[1L]]
  n <- length(chars)
  start_count <- 0L
  for (i in seq_len(n)) {
    if (chars[i] == "M") start_count <- start_count + 1L else break
  }
  end_count <- 0L
  for (i in rev(seq_len(n))) {
    if (chars[i] == "M") end_count <- end_count + 1L else break
  }
  c(start = start_count, end = end_count)
}


get_read_start_and_end_from_lex_rex_reads <- function(fp_str) {
  chars <- strsplit(fp_str, "")[[1L]]
  non_m <- which(chars != "M")
  if (length(non_m) == 0L) return(c(start = NA_integer_, end = NA_integer_))
  # Return 0-based indices matching Python
  c(start = non_m[1L] - 1L, end = non_m[length(non_m)] - 1L)
}


get_count_and_percentage_methylation <- function(mvec_substr) {
  chars <- strsplit(mvec_substr, "")[[1L]]
  # Python: skip '.'; c==c.lower() -> lowercase; c==c.upper() -> uppercase
  # 'M'.lower()='m' != 'M' so M is NOT counted as upper (skipped naturally)
  upper <- sum(grepl("[A-LN-Z]", chars))   # uppercase, M excluded
  lower <- sum(grepl("[a-z]",    chars))   # lowercase
  total <- upper + lower
  per_c <- if (total > 0L) upper / total else 0.0
  list(per_c = per_c, total_upper = upper,
       total_lower = lower, total = total)
}


# FIX 1: Rewritten to exactly match Python get_per_orange_for_each_footprint.
# Python logic:
#   for c in fvec:
#     if c == 'F':  cnt += 1
#     else:          if cnt != 0: append(round(cnt/fvec_len*100, 2)); cnt = 0
#   if cnt != 0: append(round(cnt/fvec_len*100, 2))
# Key points:
#   - Only uppercase 'F' increments counter
#   - ANY other char (dot, lowercase, other letter) triggers append+reset
#   - Denominator is TOTAL window length, not run length
get_per_orange_for_each_footprint <- function(boundary_str) {
  if (nchar(boundary_str) == 0L) return(numeric(0))
  
  chars <- strsplit(boundary_str, "")[[1L]]
  fvec_len <- length(chars)
  pct_list <- numeric(0)
  cnt <- 0L
  
  for (i in seq_along(chars)) {
    if (chars[i] == "F") {
      cnt <- cnt + 1L
    } else {
      if (cnt > 0L) {
        # Denominator MUST be the total window size (fvec_len)
        pct <- (cnt / fvec_len) * 100
        pct_list <- c(pct_list, round(pct, 2))
        cnt <- 0L
      }
    }
  }
  
  # Catch a run that terminates exactly at the window edge
  if (cnt > 0L) {
    pct <- (cnt / fvec_len) * 100
    pct_list <- c(pct_list, round(pct, 2))
  }
  
  pct_list
}

compute_state_id <- function(max_p, flen_p_list, max_s, flen_s_list) {
  # Define classification rule for Primary Track
  primary_label <- "0"
  if (length(flen_p_list) > 0) {
    max_flen_p <- max(flen_p_list)
    if (max_p > 30.0) {
      primary_label <- "3"
    } else if (max_flen_p > 50L) {
      primary_label <- "2"
    } else {
      primary_label <- "1"
    }
  }
  
  # Define classification rule for Secondary Track
  secondary_label <- "0"
  if (length(flen_s_list) > 0) {
    max_flen_s <- max(flen_s_list)
    if (max_s > 30.0) {
      secondary_label <- "3"
    } else if (max_flen_s > 50L) {
      secondary_label <- "2"
    } else {
      secondary_label <- "1"
    }
  }
  
  # Build lookup map to yield correct State ID equivalents natively
  state_matrix <- list(
    "0-0" = 0,  "1-0" = 1,  "2-0" = 2,  "3-0" = 3,
    "0-1" = 4,  "1-1" = 5,  "2-1" = 6,  "3-1" = 7,
    "0-2" = 8,  "1-2" = 9,  "2-2" = 10, "3-2" = 11,
    "0-3" = 12, "1-3" = 13, "2-3" = 14, "3-3" = 15
  )
  
  pair_key <- paste0(primary_label, "-", secondary_label)
  
  # Default fallback if combination key drops out of boundary definitions
  if (!pair_key %in% names(state_matrix)) {
    return(0)
  }
  
  return(state_matrix[[pair_key]])
}


# get_real_footprint_length_with_abs_start
# Finds runs of non-'.' chars in boundary_str.
# Returns flen_list, loc_first, abs_start (all 0-based, matching Python).
get_real_footprint_length_with_abs_start <- function(m_vec, m_vec_start, m_vec_stop, complete_vec) {
  # Locate all footprint string patterns
  matches <- gregexpr("F+", m_vec)[[1L]]
  if (matches[1L] == -1L) {
    return(list(flen_list = numeric(0), abs_start = integer(0)))
  }
  
  run_lengths <- attr(matches, "match.length")
  starts_in_m_vec <- as.integer(matches)
  
  num_runs <- length(run_lengths)
  flen_list <- numeric(num_runs)
  abs_start <- integer(num_runs)
  
  n_m_vec <- nchar(m_vec)
  n_total <- nchar(complete_vec)
  
  for (i in seq_len(num_runs)) {
    r_len <- run_lengths[i]
    s_m <- starts_in_m_vec[i]
    
    # Map back to 1-based index on complete genome vector
    a_start <- m_vec_start + (s_m - 1L)
    
    # Left flank expansion: Must hit exactly on the edge to trigger outside checking
    if (s_m == 1L) {
      left_idx <- a_start - 1L
      while (left_idx >= 1L && substr(complete_vec, left_idx, left_idx) == "F") {
        a_start <- a_start - 1L
        r_len <- r_len + 1L
        left_idx <- left_idx - 1L
      }
    }
    
    # Right flank expansion: Precise alignment matching Python window limit
    if ((s_m - 1L) + run_lengths[i] == n_m_vec) {
      right_idx <- m_vec_stop + 1L
      while (right_idx <= n_total && substr(complete_vec, right_idx, right_idx) == "F") {
        r_len <- r_len + 1L
        right_idx <- right_idx + 1L
      }
    }
    
    flen_list[i] <- r_len
    abs_start[i] <- a_start
  }
  
  list(flen_list = flen_list, abs_start = abs_start)
}


# ── SECTION 2: line-parsing helpers ──────────────────────────────────────────

find_tab_positions <- function(line) {
  # Returns 0-based tab positions (matches Python re.finditer)
  hits <- gregexpr("\t", line, fixed = TRUE)[[1L]]
  if (hits[1L] == -1L) return(integer(0))
  hits - 1L   # convert R 1-based -> 0-based
}


build_read_id <- function(line, d_loc) {
  # Python: k = "`".join([ "`".join(line[0:d_loc[7]].split()), line[d_loc[10]+1:d_loc[11]] ])
  # d_loc is 0-based stored in R 1-based vector.
  # d_loc[8] in R  = Python d_loc[7]  (8th tab, 0-based index 7)
  # d_loc[11] in R = Python d_loc[10]
  # d_loc[12] in R = Python d_loc[11]
  
  # part1: line[0:d_loc[7]] -> R substr(line, 1, d_loc[8])
  # d_loc[8] is the 0-based position of the 8th tab; chars before it = 1..d_loc[8] in R
  part1_raw  <- substr(line, 1L, d_loc[8L])
  part1_cols <- strsplit(trimws(part1_raw), "\\s+")[[1L]]
  part1      <- paste(part1_cols, collapse = "`")
  
  # part2: line[d_loc[10]+1 : d_loc[11]]
  # 0-based start = d_loc[10]+1  -> R 1-based = d_loc[10]+2 = d_loc[11] (R vec) + 2? No:
  # d_loc[11] in R stores Python d_loc[10] (the 0-based value).
  # R 1-based start = 0-based start + 1 = (d_loc[10]+1)+1 = d_loc[10]+2 = d_loc[11]+2
  # R 1-based end   = 0-based end        = d_loc[11]        = d_loc[12] in R vector
  part2_start <- d_loc[11L] + 2L
  part2_end   <- d_loc[12L]
  part2       <- substr(line, part2_start, part2_end)
  
  paste(part1, part2, sep = "`")
}


extract_last_field <- function(line, d_loc) {
  # Python: line[d_loc[-1]+1 : len(line)-1]  (readLines already strips \n)
  # 0-based start = d_loc[-1]+1  -> R 1-based = d_loc[-1]+2
  start_pos <- d_loc[length(d_loc)] + 2L
  substr(line, start_pos, nchar(line))
}


safe_substr <- function(x, start, end) {
  if (is.null(x) || nchar(x) == 0L) return("")
  start <- max(1L, start)
  end   <- min(nchar(x), end)
  if (start > end) return("")
  substr(x, start, end)
}


# ── SECTION 3: binding-state assignment ──────────────────────────────────────

PAIR_MAP <- c(
  "0-0" =  0L, "0-1" =  1L, "0-2" =  2L,
  "1-0" =  3L, "1-1" =  4L, "1-2" =  5L,
  "2-0" =  6L, "2-1" =  7L, "2-2" =  8L,
  "3-0" =  9L, "3-1" = 10L, "3-2" = 11L,
  "0-3" = 12L, "1-3" = 13L, "2-3" = 14L,
  "3-3" = 15L
)


assign_binding_labels <- function(primary_pct_orange,   secondary_pct_orange,
                                  flen_primary,          flen_secondary,
                                  abs_start_primary,     abs_start_secondary,
                                  read_start_abs,        read_end_abs,
                                  s_peak) {
  primary_label   <- NA_character_
  secondary_label <- NA_character_
  cobinding       <- FALSE
  
  has_p <- length(primary_pct_orange)   > 0L
  has_s <- length(secondary_pct_orange) > 0L
  
  # ── CASE A: footprints in BOTH windows ───────────────────────────────────
  if (has_p && has_s) {
    
    idx_p  <- which.max(primary_pct_orange)
    max_p  <- primary_pct_orange[idx_p]
    flen_p <- flen_primary[idx_p]
    abs_p  <- abs_start_primary[idx_p]
    
    idx_s  <- which.max(secondary_pct_orange)
    max_s  <- secondary_pct_orange[idx_s]
    flen_s <- flen_secondary[idx_s]
    abs_s  <- abs_start_secondary[idx_s]
    
    if (max_p > 30 && max_s > 30) {
      
      if (abs_p == abs_s) {
        is_edge <- (abs_p == read_start_abs) || ((abs_p + flen_p - 1L) == read_end_abs)
        if (!is_edge) {
          if (s_peak <= 100L) {
            if (flen_p <= 100L) {
              cobinding       <- TRUE
              primary_label   <- "1"
              secondary_label <- "1"
            } else {
              primary_label   <- "2"
              secondary_label <- "2"
            }
          } else {
            primary_label   <- "2"
            secondary_label <- "2"
          }
        } else {
          if (flen_p <= 50L) {
            primary_label   <- "3"
            secondary_label <- "3"
          } else {
            primary_label   <- "2"
            secondary_label <- "2"
          }
        }
        
      } else {
        is_edge_p <- (abs_p == read_start_abs) || ((abs_p + flen_p - 1L) == read_end_abs)
        if (!is_edge_p) {
          primary_label <- if (flen_p <= 50L) "1" else "2"
        } else {
          primary_label <- if (flen_p > 50L) "2" else "3"
        }
        
        is_edge_s <- (abs_s == read_start_abs) || ((abs_s + flen_s - 1L) == read_end_abs)
        if (!is_edge_s) {
          secondary_label <- if (flen_s <= 50L) "1" else "2"
        } else {
          secondary_label <- if (flen_s > 50L) "2" else "3"
        }
      }
      
    } else if (max_p > 30 && max_s <= 30) {
      if (abs_s != read_start_abs && (abs_s + flen_s - 1L) != read_end_abs) {
        secondary_label <- "0"
      } else {
        secondary_label <- "3"
      }
      
      is_edge_p <- (abs_p == read_start_abs) || ((abs_p + flen_p - 1L) == read_end_abs)
      if (is_edge_p) {
        primary_label <- if (flen_p > 50L) "2" else "3"
      } else {
        primary_label <- if (flen_p <= 50L) "1" else "2"
      }
      
    } else if (max_p <= 30 && max_s > 30) {
      if (abs_p != read_start_abs && (abs_p + flen_p - 1L) != read_end_abs) {
        primary_label <- "0"
      } else {
        primary_label <- "3"
      }
      
      is_edge_s <- (abs_s == read_start_abs) || ((abs_s + flen_s - 1L) == read_end_abs)
      if (is_edge_s) {
        secondary_label <- if (flen_s > 50L) "2" else "3"
      } else {
        secondary_label <- if (flen_s <= 50L) "1" else "2"
      }
      
    } else {
      if (abs_p != read_start_abs && (abs_p + flen_p - 1L) != read_end_abs) {
        primary_label <- "0"
      } else {
        primary_label <- "3"
      }
      if (abs_s != read_start_abs && (abs_s + flen_s - 1L) != read_end_abs) {
        secondary_label <- "0"
      } else {
        secondary_label <- "3"
      }
    }
    
    # ── CASE B: footprint ONLY in primary ──────────────────────────────────────
  } else if (has_p && !has_s) {
    secondary_label <- "0"
    
    idx_p  <- which.max(primary_pct_orange)
    max_p  <- primary_pct_orange[idx_p]
    flen_p <- flen_primary[idx_p]
    abs_p  <- abs_start_primary[idx_p]
    
    if (max_p > 30) {
      is_edge <- (abs_p == read_start_abs) || ((abs_p + flen_p - 1L) == read_end_abs)
      if (is_edge) {
        primary_label <- if (flen_p > 50L) "2" else "3"
      } else {
        primary_label <- if (flen_p <= 50L) "1" else "2"
      }
    } else {
      primary_label <- "0"
    }
    
    # ── CASE C: footprint ONLY in secondary ────────────────────────────────────
  } else if (!has_p && has_s) {
    primary_label <- "0"
    
    idx_s  <- which.max(secondary_pct_orange)
    max_s  <- secondary_pct_orange[idx_s]
    flen_s <- flen_secondary[idx_s]
    abs_s  <- abs_start_secondary[idx_s]
    
    if (max_s > 30) {
      is_edge <- (abs_s == read_start_abs) || ((abs_s + flen_s - 1L) == read_end_abs)
      if (is_edge) {
        secondary_label <- if (flen_s > 50L) "2" else "3"
      } else {
        secondary_label <- if (flen_s <= 50L) "1" else "2"
      }
    } else {
      secondary_label <- "0"
    }
    
    # ── CASE D: no footprints in either window ──────────────────────────────────
  } else {
    primary_label   <- "0"
    secondary_label <- "0"
  }
  
  list(primary   = primary_label,
       secondary = secondary_label,
       cobinding = cobinding)
}


# ── SECTION 4: verbose-file parser ───────────────────────────────────────────

open_input <- function(path) {
  if (grepl("\\.gz$", path, ignore.case = TRUE)) {
    gzcon(gzfile(path, open = "rb"))
  } else {
    file(path, open = "r")
  }
}

read_all_lines <- function(path) {
  con <- open_input(path)
  on.exit(close(con))
  readLines(con, warn = FALSE)
}


parse_verbose_file <- function(verbose_file) {
  message("  Reading verbose file: ", verbose_file)
  lines     <- read_all_lines(verbose_file)
  n_records <- length(lines) %/% 3L
  if (n_records == 0L) stop("Verbose file appears empty or malformed.")
  
  verb_dict  <- vector("list", n_records)
  names_vec  <- character(n_records)
  type_names <- c("footprint", "mvec", "bs_seq")
  
  for (i in seq_len(n_records)) {
    base <- (i - 1L) * 3L
    
    line0  <- lines[base + 1L]
    d_loc0 <- find_tab_positions(line0)
    key    <- build_read_id(line0, d_loc0)
    
    rec        <- vector("list", 3L)
    names(rec) <- type_names
    for (j in 1:3) {
      raw      <- lines[base + j]
      d_loc    <- find_tab_positions(raw)
      rec[[j]] <- extract_last_field(raw, d_loc)
    }
    
    verb_dict[[i]] <- rec
    names_vec[i]   <- key
  }
  
  names(verb_dict) <- names_vec
  verb_dict
}


# ── SECTION 5: main processing ───────────────────────────────────────────────

assign_cobinding_states_R <- function(input_file,
                                      verbose_file,
                                      lflank, rflank,
                                      lextend, rextend,
                                      output_file,
                                      output_verbose,
                                      output_150bp) {
  
  # 1. Parse verbose file
  verb_dict <- parse_verbose_file(verbose_file)
  
  # 2. Read main input
  message("  Reading input file:   ", input_file)
  input_lines <- read_all_lines(input_file)
  input_lines <- input_lines[nchar(trimws(input_lines)) > 0L]
  message("  Records to process:   ", length(input_lines))
  
  # 3. Open gzip output connections
  con_fp      <- gzfile(output_file,    "wb")
  con_verbose <- gzfile(output_verbose, "wb")
  con_150     <- gzfile(output_150bp,   "wb")
  on.exit({ close(con_fp); close(con_verbose); close(con_150) }, add = TRUE)
  
  # 4. Process each read
  for (line_idx in seq_along(input_lines)) {
    line  <- input_lines[line_idx]
    d_loc <- find_tab_positions(line)
    
    # s_peak: Python line[d_loc[6]+1 : d_loc[7]]
    # R: d_loc[7] = Python d_loc[6], d_loc[8] = Python d_loc[7]
    # R 1-based start = d_loc[7]+2, R 1-based end = d_loc[8]
    s_peak <- suppressWarnings(as.integer(substr(line, d_loc[7L] + 2L, d_loc[8L])))
    if (is.na(s_peak)) next
    
    read_id <- build_read_id(line, d_loc)
    fp_str  <- extract_last_field(line, d_loc)
    
    # Boundary indices (0-based, matching Python exactly)
    primary_lf   <- lextend - lflank
    primary_rf   <- lextend + rflank + 1L
    secondary_lf <- lextend + s_peak - lflank
    secondary_rf <- lextend + s_peak + rflank + 1L
    
    # Read extent (0-based)
    read_bounds <- get_read_start_and_end_from_lex_rex_reads(fp_str)
    if (is.na(read_bounds["start"]) || is.na(read_bounds["end"])) next
    read_start_abs <- read_bounds["start"]
    read_end_abs   <- read_bounds["end"]
    
    # M-replacement & boundary substrings
    # Python: m_replaced_str = fp_str.replace("M", ".")
    # Python slice [a:b] -> R substr(s, a+1, b)
    m_replaced_str <- gsub("M", ".", fp_str, fixed = TRUE)
    primary_str    <- substr(m_replaced_str, primary_lf    + 1L, primary_rf)
    secondary_str  <- substr(m_replaced_str, secondary_lf + 1L, secondary_rf)
    
    # Footprint lengths & absolute starts
    res_p <- get_real_footprint_length_with_abs_start(
      primary_str,   primary_lf,   primary_rf,   m_replaced_str)
    res_s <- get_real_footprint_length_with_abs_start(
      secondary_str, secondary_lf, secondary_rf, m_replaced_str)
    
    # % orange per F-run (FIXED: uses corrected function)
    primary_pct_orange   <- get_per_orange_for_each_footprint(primary_str)
    secondary_pct_orange <- get_per_orange_for_each_footprint(secondary_str)
    
    # Methylation (computed to mirror Python; results not used in label logic)
    rec <- verb_dict[[read_id]]
    if (is.null(rec)) {
      warning("Missing read_id in verbose file: ", read_id, call. = FALSE)
      next
    }
    
    mvec_str <- rec[["mvec"]]
    bs_str   <- rec[["bs_seq"]]
    
    if (!is.null(mvec_str)) {
      # Python: mvec[primary_lf-15 : primary_rf]   (0-based slice)
      # R substr: start=primary_lf-15+1, end=primary_rf
      get_count_and_percentage_methylation(
        safe_substr(mvec_str, primary_lf - 15L + 1L, primary_rf))
      get_count_and_percentage_methylation(
        safe_substr(mvec_str, primary_lf + 1L,        primary_rf + 15L))
      get_count_and_percentage_methylation(
        safe_substr(mvec_str, secondary_lf - 15L + 1L, secondary_rf))
      get_count_and_percentage_methylation(
        safe_substr(mvec_str, secondary_lf + 1L,        secondary_rf + 15L))
    }
    
    # Assign labels
    labels <- assign_binding_labels(
      primary_pct_orange   = primary_pct_orange,
      secondary_pct_orange = secondary_pct_orange,
      flen_primary         = res_p$flen_list,
      flen_secondary       = res_s$flen_list,
      abs_start_primary    = res_p$abs_start,
      abs_start_secondary  = res_s$abs_start,
      read_start_abs       = read_start_abs,
      read_end_abs         = read_end_abs,
      s_peak               = s_peak
    )
    
    label_comb  <- paste0(labels$primary, "-", labels$secondary)
    one_of_nine <- PAIR_MAP[[label_comb]]
    if (is.null(one_of_nine)) {
      warning("Unknown label combination '", label_comb,
              "' for read: ", read_id, call. = FALSE)
      next
    }
    
    # Build output strings
    # Python: modified_fp_str = fp_str  (no M-replacement for output!)
    key_tag  <- paste0(read_id, "#", one_of_nine)
    mvec_out <- if (!is.null(mvec_str)) mvec_str else ""
    bs_out   <- if (!is.null(bs_str))   bs_str   else ""
    
    fp_line   <- paste0(key_tag, "\t", fp_str)
    mvec_line <- paste0(key_tag, "\t", mvec_out)
    bs_line   <- paste0(key_tag, "\t", bs_out)
    
    # 150bp centre trim
    # Python: centre = int((len(modified_fp_str)-1)/2)  where modified_fp_str=fp_str
    # slice [centre-150 : centre+151]  (Python exclusive end = 301 chars)
    fp_len   <- nchar(fp_str)
    centre   <- as.integer((fp_len - 1L) / 2L)   # 0-based centre
    t_start  <- centre - 150L                      # 0-based inclusive start
    t_end    <- centre + 150L                      # 0-based inclusive end
    # R substr 1-based: start = t_start+1, end = t_end+1
    fp_150_str   <- safe_substr(fp_str,   t_start + 1L, t_end + 1L)
    mvec_150_str <- safe_substr(mvec_out, t_start + 1L, t_end + 1L)
    bs_150_str   <- safe_substr(bs_out,   t_start + 1L, t_end + 1L)
    
    fp_line_150   <- paste0(key_tag, "\t", fp_150_str)
    mvec_line_150 <- paste0(key_tag, "\t", mvec_150_str)
    bs_line_150   <- paste0(key_tag, "\t", bs_150_str)
    
    # Write outputs (order matches Python exactly)
    writeLines(fp_line,                               con_fp)
    writeLines(c(fp_line, mvec_line, bs_line),        con_verbose)
    writeLines(c(fp_line_150, mvec_line_150, bs_line_150), con_150)
  }
  
  message("  Written: ", output_file)
  message("  Written: ", output_verbose)
  message("  Written: ", output_150bp)
  invisible(NULL)
}


# ── SECTION 6: CLI entry point ────────────────────────────────────────────────

args <- commandArgs(trailingOnly = TRUE)

if (length(args) != 9L) {
  cat("Usage: Rscript assign_cobinding_states.R",
      "<input.bedpe.gz> <verbose.bedpe.gz>",
      "<lflank> <rflank> <lextend> <rextend>",
      "<out.bedpe.gz> <out_verbose.bedpe.gz> <out_150bp.bedpe.gz>\n")
  quit(status = 1L)
}

input_file     <- args[1L]
verbose_file   <- args[2L]
lflank         <- as.integer(args[3L])
rflank         <- as.integer(args[4L])
lextend        <- as.integer(args[5L])
rextend        <- as.integer(args[6L])
output_file    <- args[7L]
output_verbose <- args[8L]
output_150bp   <- args[9L]

for (out_path in c(output_file, output_verbose, output_150bp)) {
  out_dir <- dirname(out_path)
  if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)
}

message("=== assign_cobinding_states ===")
message("  input        : ", input_file)
message("  verbose      : ", verbose_file)
message("  lflank/rflank: ", lflank, " / ", rflank)
message("  lextend/rext : ", lextend, " / ", rextend)

assign_cobinding_states_R(
  input_file     = input_file,
  verbose_file   = verbose_file,
  lflank         = lflank,
  rflank         = rflank,
  lextend        = lextend,
  rextend        = rextend,
  output_file    = output_file,
  output_verbose = output_verbose,
  output_150bp   = output_150bp
)

message("=== Done ===")